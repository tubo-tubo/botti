...
class JudgeGoal():
    ...
    def __init__(self, width=400, height=267, selectcolor_judge=110, othercolor1_judge=100, othercolor2_judge=100):
        self.width = width
        self.height = height
...
class flagcapture(object):
    ...
    def judge(self):
        judgeimage = JudgeGoal(width=1920, height=1080)
        judgeimage.loadimage(filename=self.imagename)
        judgeimage.objectselect()
        return judgeimage.objectdirection()

if __name__ == '__main__':
    judge = JudgeGoal()
    judge.loadimage(filename='./colorcone2.jpg')
    judge.objectselect()
